...
class MainPage
	constructor : ->
		...
		w = width
		h = height
		@buttons['left'] =  new MainButton 0,  0.5*w,0.225*h,1.00*w,0.45*h, left,  true
		@buttons['right'] = new MainButton 1,  0.5*w,0.775*h,1.00*w,0.45*h, right, true
		@buttons['play'] =  new Button 'play', 0.1125*w,0.50*h,0.225*w,0.10*h, pause, true,  true
		@buttons['pause'] = new Button 'pause',(0.1125+0.225)*w,0.50*h,0.225*w,0.10*h, pause, false, true
		@buttons['new'] =   new Button 'new',  0.8*w,0.50*h,0.45*w,0.10*h, edit,  true,  true
...
class MainButton extends Button
	...
	draw : ->
		...
		if state == 0 and player==-1
			fill 'yellow'
		else if state == 1 
			fill 'white'
		else
			fill if player==@player then 'white' else "yellow"
		rect @x,@y,@width,@height
